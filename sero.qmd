...
sp1222 <- out_pava %>% filter(month(col_date) == 12 & year(col_date) == 2022) %>% 
  group_by(age) %>% 
  summarise(
...
sp0423 <- out_pava %>% filter(month(col_date) == 04 & year(col_date) == 2023) %>% 
  group_by(age) %>% 
  summarise(
    sp = mean(seroprev_monotonic),
  ) %>% ggplot(aes(x = age,y = sp))+
...
sp0823 <- out_pava %>% filter(month(col_date) == 08 & year(col_date) == 2023) %>% 
  group_by(age) %>% 
  summarise(
    sp = mean(seroprev_monotonic),
  ) %>% ggplot(aes(x = age,y = sp))+
...
sp1223 <- out_pava %>% filter(month(col_date) == 12 & year(col_date) == 2023) %>% 
  group_by(age) %>% 
  summarise(
    sp = mean(seroprev_monotonic),
  ) %>% ggplot(aes(x = age,y = sp))+
