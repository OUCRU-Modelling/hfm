...
## Using k-means to find the time point that separates two peaks

```{r}
set.seed(123)

df23 %<>% mutate(wave = data.frame(age1, time) |> 
                   kmeans(centers = 2, nstart = 25) |> 
                   extract2("cluster"))
```

```{r}
with(df23, plot(admission_date, age1, col = adjustcolor(wave + 1, .1), ylim = c(0, 10),
                xlab = NA, ylab = "age (years)"))
```
...
```{r}
with(df23, plot_dens(density(age1), xlim = c(0, 10)))
```
...
df23$mclust <- a$classification
```

```{r}
with(df23, plot(admission_date, age1, col = adjustcolor(mclust, .1), ylim = c(0, 10),
                xlab = NA, ylab = "age (years)"))
```
...
pnorm1 <- function(q) pnorm(q, mclust_out$mean[1], sqrt(mclust_out$variance$sigmasq[1]), FALSE)
pnorm2 <- function(q) pnorm(q, mclust_out$mean[2], sqrt(mclust_out$variance$sigmasq[2]))

xs <- sort(df23$time)
plot(xs, pnorm1(xs), type = "l")
lines(xs, pnorm2(xs))
...
### Try to model virology as a function of age and time {.unnumbered}
