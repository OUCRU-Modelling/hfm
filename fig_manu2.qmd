...
library(ISOweek)
count_dangky_week <- readRDS("D:/OUCRU/hfmd/hcm_birth_data.rds")
child <- count_dangky_week %>% filter(birth_year >= 2017) %>% group_by(birth_week, birth_year) %>%
  summarise(n=sum(n),.groups = "drop") %>% arrange(birth_year)
...
# constrained_age_profiles_cohort2_cm <- age_profile_constrained_cohort2(hfmd)
# 
...
c_ad <- readRDS("D:/OUCRU/hfmd/data/CH1_out_cleaned.rds")

hcm_birth_data <- count_dangky_week
...
pop_23_fn <- predicted_birth_district %>% 
  group_by(district_reg,birth_year) %>% 
  summarise(birth = sum(predicted), .groups = "drop") %>% 
  mutate(age_23 = 2023 - birth_year) %>% 
  filter() %>% 
  full_join(.,N_ad %>% 
              mutate(age_23 = 2023-2019+age),
            by = join_by(age_23,district_reg == district)) %>% 
  mutate(pop_final = case_when(
    is.na(n) ~ birth,
    !is.na(n) ~ n
  )) %>% 
  select(district_reg,age_23,pop_final)
...
#| out-width: "100%"
#| fig-format: png
#| fig-cap: "The points show the original hospitalization, and the lines show fitted value of GAM"

expected_catchment_CH1 <- left_join(c_ad,pop_23_fn,by = join_by(district == district_reg,
                                      age_gr == age_23)) %>% 
  mutate(p_hos = n/pop_final) %>% 
  arrange(age_gr) %>% 
  group_by(age_gr) %>% 
  mutate(weight = p_hos/sum(p_hos),
         expected_cm = n*weight) %>% 
  ungroup() 

expected_age_cm <- expected_catchment_CH1  %>% 
  group_by(age_gr) %>% 
  summarise(e_admission = sum(expected_cm)) %>%
  gam(e_admission~s(age_gr,bs = "cs"),method = "REML",data = .) %>% 
  predict(list(age_gr = seq(0,16,le = 512)),type="response",se.fit = TRUE) %>%
  as.tibble() %>% 
...
```{r}
### Expected incidence with 95% CI

map2(head(age_pro5, -1),
     age_pro5[-1],
     ~ left_join(na.exclude(.x), na.exclude(.y), "cohort")|>
       mutate(sp_gap = (fit.y - fit.x),
              se.x = (upr.x - lwr.x) / (2 * 1.96),
              se.y = (upr.y - lwr.y) / (2 * 1.96),
              se.diff = sqrt(se.x^2 + se.y^2),
              fit.diff = fit.y - fit.x,
              lwr.diff = fit.diff - 1.96 * se.diff,
              upr.diff = fit.diff + 1.96 * se.diff)) %>% 
  # bind_rows(.id = "id") %>% 
  map(~ left_join(.x,expected_age_cm, by = join_by(cohort == age))) %>% 
  bind_rows(.id = "id") %>% 
  mutate(exp_in = sp_gap*fit,
         exp_in_upr = upr.diff*upr,
         exp_in_lwr = lwr.diff*lwr) %>% 
  ggplot()+
  geom_line(aes(x = cohort,y = exp_in))+
  geom_ribbon(aes(x = cohort,y = exp_in,
                  ymin = exp_in_lwr,ymax = exp_in_upr),fill = "blue",alpha = .3)+
  geom_histogram(data = dat %>% bind_rows(.id = "id") %>% filter(cohort >0),
                 aes(cohort),binwidth = 0.5,
                 color = "white",fill = "black",alpha = 0.2)+
  facet_wrap(~factor(id,labels = c("12/2022 - 4/2023",
  ...
  coord_cartesian(ylim = c(0,1000), expand = c(0,0))+
  theme_bw()+
  theme(
    axis.text.x = element_text(size = 15),
    axis.text.y = element_text(size = 18),
...
lab_ev71_p <- viro2 %>%
  ...
  scale_y_continuous(name = " EV-A71 Percentage (%)",
                     labels = scales::label_percent(scale = 100),
                     # position = "right"
                     )+
  labs(x = "Age (years)",tag = "F")+
  facet_wrap(~peak)+
  theme_bw()+
  theme(axis.title.x = element_text(size = 18),
        axis.text.x = element_text(size = 18),
...
row1 <- (age_profile_sp_cm | lab_ev71_p)+
  plot_layout(widths = c(2.6, 1.9))


row2 <-  ((ts/hm2) | (plot_spacer()/
                        wrap_elements(full = ad  + labs(tag = "G")+
