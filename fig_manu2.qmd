...
OUCRU serum bank from Children hospital 1, so we calculate the catchment area of CH1 by cases ratio method
...
The correlation plots show that there was a correlation between the number of cases and the number of serum samples in the catchment area. So we used the sero data to estimate the age profile of cumulative incidence in the CH1 catchment area.
...
#### December 2022 - Apr 2023 {.unnumbered}
...
corr_sero_numcases <- function(timepoint){
  
  date_range <- data_pt %>%
    filter(col_time %in% timepoint & 
             qhchuan %in% district_consider) %>%
    pull(col_date) %>%
    range()
  ...
  group_by(district2) %>%
  count() %>%
  left_join(.,num_sero, by = join_by(district2 == qhchuan))
}
```

```{r}

corr_sero_numcases(timepoint = c("Apr 2023","Dec 2022")) %>%
  ggscatterstats(
    x = n.x,
    y = n.y,
    bf.message = FALSE,
    marginal = FALSE,
    label.var = district2,
    xlab = "Number of HFMD cases in Dec 2022 - Apr 2023",
    ylab = "Number of serum samples in Dec 2022 - Apr 2023"
  )
```

#### Apr 2023 - Aug 2023 {.unnumbered}

```{r}
corr_sero_numcases(timepoint = c("Apr 2023","Aug 2023")) %>%
  ggscatterstats(
    x = n.x,
    y = n.y,
    bf.message = FALSE,
    marginal = FALSE,
    label.var = district2,
    xlab = "Number of HFMD cases in Apr 2023 - Aug 2023",
    ylab = "Number of serum samples in Apr 2023 - Aug 2023"
  )
```

#### Aug 2023 - Dec 2023 {.unnumbered}

```{r}
corr_sero_numcases(timepoint = c("Dec 2023","Aug 2023")) %>%
  ggscatterstats(
    x = n.x,
    y = n.y,
    bf.message = FALSE,
    marginal = FALSE,
    label.var = district2,
    xlab = "Number of HFMD cases in Aug 2023 - Dec 2023",
    ylab = "Number of serum samples in Aug 2023 - Dec 2023"
  )
```
